...
// 1. Accept onStockClick as a prop
const PortfolioSharesList = ({heldShares, removeHeldSharesInCompany, removeSomeSharesInCompany, addSomeSharesInCompany, onStockClick }) => {

    const portfolioSharesItems = heldShares.map((heldShare, index) => {
        // 2. Pass the handler down to each item
        return <PortfolioSharesItem 
                    heldShare={heldShare} 
                    key={index} 
