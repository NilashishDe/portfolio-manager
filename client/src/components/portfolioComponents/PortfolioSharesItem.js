...
import { deleteShares, editCurrentSharesDB } from "../../services/PortfolioServices";
import ChartHoldingPriceHistory from "../sharedComponents/ChartHoldingPriceHistroy";

const PortfolioSharesItem = ({heldShare, removeHeldSharesInCompany, removeSomeSharesInCompany, addSomeSharesInCompany, onStockClick }) => {
    ...
    const handleDelete = () => {
        // --- MODIFIED: Pass currentPrice in payload for accurate logging ---
        const payload = { currentPrice: heldShare.currentPrice };
        deleteShares(heldShare._id, payload) 
        .then(() => {
            removeHeldSharesInCompany(heldShare._id)
        })
        handleCloseDelete()
    }
    ...
    const handleRemove = () => {
        ...
        const shares = {
            ...
            currentPrice: heldShare.currentPrice 
        }

        if (newNumShares <= 0){
            deleteShares(heldShare._id, { currentPrice: heldShare.currentPrice })
            .then(() => {
                removeHeldSharesInCompany(heldShare._id)
            })
        }else{
    ...
    return (  
        <>
            <tr onClick={() => onStockClick(heldShare.symbol)} style={{cursor: 'pointer'}}>
                <td>{heldShare.symbol}</td>
                <td onMouseEnter={() => setIsHovered(true)} onMouseLeave={() => setIsHovered(false)}>
                    <span style={nameStyle}>{heldShare.name}</span>
                </td>
                <td>{heldShare.numberOfShares}</td>
                <td>${heldShare.avgPurchasePrice}</td>
                <td>${heldShare.currentPrice}</td>
                <td>${totalPaidPrice}</td>
                <td>${totalValue}</td>
                ...
                <td>
                    <Button variant="info" onClick={(e) => handleButtonClick(e, handleShowChart)}>
                        <AiOutlineAreaChart/>
                    </Button>
                    <Button variant="success" onClick={(e) => handleButtonClick(e, handleShowAddMoreHeldShares)}>
                        <AiFillFileAdd />
                    </Button>
                    <Button variant="warning" onClick={(e) => handleButtonClick(e, handleShowRemoveSomeHeldShares)}>
                        <HiDocumentRemove />
                    </Button>
                    <Button variant="danger" onClick={(e) => handleButtonClick(e, handleShowDelete)}>
                        <BsFillTrashFill/>
                    </Button>
                </td>
            </tr>
            
            <Modal show={showChart} onHide={handleCloseChart} keyboard={false}>
            ...
            <Modal show={showAddMoreHeldShares} onHide={handleCloseAddMoreHeldShares} backdrop="static" keyboard={false}>
                <Modal.Header closeButton>
                    ...
                    <Form>
                        ...
                        <Form.Group className="mb-3">
                            ...
                            <Form.Text className="text-muted">
                                ...
                                <p>Current Average Price Paid: ${heldShare.avgPurchasePrice}</p>
                            </Form.Text>
                        </Form.Group>
                    </Form>
                </Modal.Body>
                <Modal.Footer>
                    <Button variant="secondary" onClick={handleCloseAddMoreHeldShares}>Cancel</Button>
                    <Button onClick={handleAdd} variant="success" type="submit">Add</Button>
                </Modal.Footer>
            </Modal>    

            <Modal show={showRemoveSomeHeldShares} onHide={handleCloseRemoveSomeHeldShares} backdrop="static" keyboard={false}>
                <Modal.Header closeButton>
                ...
                <Modal.Footer>
                    <Button variant="secondary" onClick={handleCloseRemoveSomeHeldShares}>Cancel</Button>
                    <Button onClick={handleRemove} variant="danger" type="submit">Remove</Button>
                </Modal.Footer>
            </Modal>    
        </>
    );
